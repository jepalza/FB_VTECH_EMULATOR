' Emulador Z80 retocado para que funcione en el Prestige-Elite

' boleanas
Dim Shared Falso As Long =  0
Dim Shared Verdadero  As long =  -1
...
' conversion a hexadecimal formateado en 4 cifras
Declare Function hexdec(a As Long) As string
...
Dim Shared glTStates As Long 
Dim Shared interruptCounter As Long
Dim Shared glInterruptDelay As Long
Dim Shared glDelayOverage As Long = 0 ' necesario ponerla a "0" la primera vez

' tabla de divisiones, mas rapido que dividir en tiempo real 
Dim Shared Div4    (81919) As Long
Dim Shared Div32   (81919) As Long
Dim Shared Div256  (81919) As Long 
Dim Shared Div16384(81919) As Long 
...
Function HexDec(a As Long ) As String
	HexDec=right("0000"+Hex(a),4)
End Function
...
Sub daa_a()
    ...
    If (fH = Verdadero) Or ((regA And &HF&) > &H9&) Then
        incr = incr Or &H6&
    End If
    
    If (fC = Verdadero) Or (regA > &H9F&) Then
        incr = incr Or &H60&
    End If
    ...
    If (regA > &H99&) Then fC = Verdadero
    
    If (fN = Verdadero) Then
        ' sub_a incr
...
Sub execute()
    ...
    While Verdadero
      ...
      If deb then 
       	...
       	' en modo depuracion (DEB=1) podemos parar cuando queramos con la tecla espacio
       	if inkey=" " then BreakPoint=1
      
         locate 1,1
         Print "Ciclos=";Abs(glTstates);"       "
         Print
         print "PC="+hexdec(regpc)+" : "+Right(HexDec(peekb(regpc)),2)+" "+Right(hexdec(peekw(regpc+1)),2)+" "+left(hexdec(peekw(regpc+1)),2)
         print "SP="+HexDec(regsp)
         print
         print "A ="+Right(HexDec(regA),2)
         print "BC="+right(HexDec(regB),2)+Right(HexDec(regC),2)
         print "DE="+HexDec(regDE)
         print "HL="+HexDec(regHL)
         print
         print "AF'="+HexDec(regAF_)+"    ","IX="+HexDec(regIX)
         print "BC'="+HexDec(regBC_)+"    ","IY="+HexDec(regIY)
         print "DE'="+HexDec(regDE_)'''' +"    ","ID="+HexDec(regID)
         print "HL'="+HexDec(regHL_)
         Print
         Print "notas: FALSO=0 -- VERDADERO=-1"
         Print "IM1=";intIM
         Print "IFF1=";intIFF1
         Print "IFF2=";intIFF2
         Print
         If FS Then sA=sA+"X" Else sA=sA+"."
			If FZ Then sA=sA+"X" Else sA=sA+"."
			If F5 Then sA=sA+"X" Else sA=sA+"."
			If FH Then sA=sA+"X" Else sA=sA+"."
			If F3 Then sA=sA+"X" Else sA=sA+"."
			If FPV Then sA=sA+"X" Else sA=sA+"."
			If FN Then sA=sA+"X" Else sA=sA+"."
			If FC Then sA=sA+"X" Else sA=sA+"."
			Print "SZYHXPNC"
			Print sA
			Locate 20,20:For d=&h50 To &h56:Print puerto(d);:next
         
         ' con ENTER, podemos meter otro punto de paro (En HEX)
         if BreakPoint=1 then 
         	 sA=""
         	 While sA="":sA=InKey:Wend 
         	 If sA=Chr(27) Then 
         	 ...
         	 if sA=Chr(13) then 
          	  Cls
          	  Input "Nueva direccion de paro (HEX):";sA
          	  DirParar=Val("&h"+sA)
          	  BreakPoint=0
          	  cls
         	 EndIf
...
Function peekb(addr As long) As long
    addr=addr and &hffff
    if addr=DirParar+1 then BreakPoint=1
...
Function peekw(addr As long) As long
    peekw = peekb(addr) or (256 * peekb(addr+1))
...
Function interrupt() As long
    ...
    ' por defecto, si no se indica nada, sale "0"
    interrupt=0
     
    ' // PERFORM Z80 hardware functions
    interruptCounter = interruptCounter + 1
    
    ' // state alters every 1/2 second
    If (interruptCounter Mod 16)=0 Then
    	
    	      'DoEvents
    	      'glTstatesPerInterrupt+=1
    	      'gltstates=-glTstatesPerInterrupt
        	   'Locate 40,70:Print glTstatesPerInterrupt
        	   'Z80Reset
            'For f=0 to 65535:puerto(f)=0: Next
            'for f=0 to &h2000*4:mem(f)=0:Next
            'For f=0 to 65535:RAM(f)=0:Next
            'copiarom(&h0000,&h0)
            'interrupt = 0
            'If (Point(250+3,380+8)+Point(250+5,380+8))=18 Then Sleep
            ''Line(250+5,380+8)-(250+5,380+20),1,BF:SLEEP
    	      ''DoEvents
    	      '
    	      'Return interrupt
    ...
    End If
     ...
     If intIFF1 = Verdadero Then
     	'If sig_ins = 1 Then
      	intIFF1=0
      	intIFF2=0
      	'sig_ins=0
         If irq_counter=2 Then
         	irq_counter=0
         	pushpc
         	regpc=&h20
    	      interrupt=13
   ...
   ' miramos el tiempo transcurrido
    ...
    ' limites admitidos de errores en la velocidad (en milisegundos)
    If lSleep < 1 Then lSleep = 1
    If lsleep > 300 Then lsleep = 300

    ' si se ha tardado menos de "glinterruptdelay" milisegundos, entonces, lo frenamos
    ...
    ' OJO: no poner RETURN INTERRUPT, ya que , se llama a si mismo 
    ' y se queda en bucle infinito  
    ' si hay que devolver una variable, usar una temporal
    ' sino, emplear un Return "valor" , como sigue...      
    'Return 56
